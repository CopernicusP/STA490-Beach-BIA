...
# Search across multiple roots (prefer main repo layout first)
DATA_DIRS <- unique(c(
  file.path(getwd(), "Data"),
  file.path(getwd(), "Dataset"),  # backward compatibility
  file.path(getwd(), "data"),
  getwd(),
  "/mnt/data"
))
...
inat_gj <- file.path(RES_DIR,"inat_aoi.geojson")
tree_gj <- file.path(RES_DIR,"trees_aoi.geojson")
aoi_gj  <- file.path(RES_DIR,"aoi_union_buffer.geojson")

use_cached <- file.exists(inat_gj) && file.exists(tree_gj) && file.exists(aoi_gj)

if (use_cached){
  msg("✅ Using cached GeoJSON in Results/")
...
summary_tbl
write_csv(summary_tbl, file.path(FIG_DIR, "bio_summary_final.csv"))
...
write_csv(top_species_tbl, file.path(FIG_DIR, "top_species.csv"))
write_csv(top_observers_tbl, file.path(FIG_DIR, "top_observers.csv"))
...
write_csv(cat_summary, file.path(FIG_DIR, "category_summary.csv"))
```
...
summary_by_area
write_csv(summary_by_area, file.path(FIG_DIR, "bio_summary_by_area.csv"))
```
...
write_csv(neigh_summary, file.path(FIG_DIR, "neighbourhood_biodiversity_summary.csv"))
neigh_summary
...
write_csv(near_flag, file.path(FIG_DIR, "near_water_by_category.csv"))
...
title="Layers")
m
saveWidget(m, file.path(RES_DIR,"map_categories_and_trees.html"), selfcontained=TRUE)
...
saveWidget(m_heat, file.path(RES_DIR,"map_heat.html"), selfcontained=TRUE)
}
```
...
m_top
saveWidget(m_top, file.path(RES_DIR,"map_top10_species.html"), selfcontained=TRUE)
```
...
m_year
saveWidget(m_year, file.path(RES_DIR,"map_by_year.html"), selfcontained=TRUE)
}
```
...
write_csv(top_by_area, file.path(FIG_DIR, "top5_species_by_area.csv"))
top_by_area
```
...
"CSVs saved in /Figures; maps + GeoJSON saved in /Results"
),
done = TRUE
)
...
write_csv(dict, file.path(FIG_DIR, "outputs_data_dictionary.csv"))
```
