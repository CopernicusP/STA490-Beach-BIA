...
> _Synced to the `main` branch to keep the consolidated biodiversity analysis up-to-date._
...
summary_by_area <- bind_rows(
  ...
  summarise_area("AOI buffer (BIA∪Woodbine)", aoi_buf)
)
summary_by_area
...
# 8) Map — category layers + trees
```{r}
m <- leaflet(options = leafletOptions(minZoom=10)) %>%
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(data=aoi_buf, label="AOI (BIA ∪ Woodbine) buffer",
...
areas_list <- list(
  ...
  `AOI buffer (BIA∪Woodbine)` = aoi_buf
)
