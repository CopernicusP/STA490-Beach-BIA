...
# Purpose: Build the Beach BIA biodiversity EDA pipeline (AOI definition, filtering, basic stats, and map).
...
# Outputs: outputs/biodiversity_summary.csv (plus figures/maps in the knitted HTML).
# Notes  : AOI = (Beach BIA ∪ Woodbine Beach) buffered by 30m; CRS standardized to EPSG:4326; buffering done in EPSG:32617.
# Author : Peter
```
...
# --- Project paths ---
# Assumes working directory is the project root (or a folder that contains the data subfolders).
# Outputs are written to <DATA_DIR>/outputs.
...
# --- Robust file resolvers ---
# We search for expected datasets by filename pattern so this notebook can run even
# if files are nested inside subfolders (as long as names are consistent).

# Prefer .geojson when available; otherwise choose a complete .shp with required sidecars (.shx/.dbf).
