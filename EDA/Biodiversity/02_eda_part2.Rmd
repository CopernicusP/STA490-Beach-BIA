---
title: "Beach BIA — Biodiversity EDA (Part 2)"
...
if (use_cached) {
  msg("✅ Using cached GeoJSON from outputs/")
  inat_beach  <- safe_read_sf(inat_gj)
  trees_beach <- safe_read_sf(tree_gj)
  aoi_buf     <- safe_read_sf(aoi_gj)
} else {
  msg("ℹ No cached GeoJSON found. Rebuilding from raw datasets…")
  ...
  msg("💾 Saved GeoJSON to outputs/ for future runs.")
}
...
addPolygons(data = aoi_buf, label = "AOI (BIA ∪ Woodbine) 30m buffer",
